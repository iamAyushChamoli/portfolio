...
const Navbar = () => {
  const [toggle, setToggle]=useState.apply(false);
    ...
    <div className='w-full flex py-6 justify-between items-center navbar'>
      ...
      <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
        {navLinks.map((nav,index)=>(
          <li key={nav.id}
          className={`font-poppins font-normal hover:text-secondary cursor-pointer text-[16px] text-white ${index===navLinks.length -1? 'mr-0': 'mr-10'}`}>
            <a href={`#${nav.id}`}>
              {nav.title}
            </a>  
          </li>
